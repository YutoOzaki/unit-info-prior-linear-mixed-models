...
x = rng.normal(loc=-5.342, scale=4.932, size=N)
mu_0 = 0
sgm_0 = 10
nu_0 = 2
K = 10
...
def main(beta):
    with pm.Model():
        ...
        #pm.Normal("likelihood", mu=p_mu, sigma=p_sgm, observed=x)**beta
        #idata = pm.sample(draws=M, chains=J)
        idata = pmj.sample_numpyro_nuts(draws=M, tune=1000, chains=J)
        return idata
